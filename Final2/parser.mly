...
expr : 
 ...
 | dec {Declaration( $1 )}
 | wordExpr CONCAT languageExpr { raise (Path.Invalid_concatination(Path.solve_word($1), $3)) }
 | languageExpr CONCAT wordExpr { raise (Path.Invalid_concatination(Path.solve_word($3), $1)) }
;

dec:
 | LANG_VAR EQUALS languageExpr { LangDeclaration ($1, $3) }
 | WORD_VAR EQUALS wordExpr { WordDeclaration ($1, $3) }
 | LANG_VAR EQUALS wordExpr { raise (Path.Invalid_declaration("Language variable("^$1^") cannot store word")) }
 | WORD_VAR EQUALS languageExpr {raise (Path.Invalid_declaration("Word variable("^$1^") cannot store language")) }
;
...
languageExpr :
 ...
 | wordExpr STAR 	{ Star(LangLiteral(Cons ($1, Empty_Set)))}
;
